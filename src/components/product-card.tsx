...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card" data-od-id={`product-card-${product.id}`}>
    ...
    <div className="product-body"><p className="product-brand">{product.brand}</p><h3><Link href={`/product/${product.slug}`}>{product.name[locale]}</Link></h3><p className="product-rating" aria-label="4.8 out of 5">★ 4.8 <span>· verified</span></p><div className="price-row"><strong>{formatBDT(product.price)}</strong>{product.compareAtPrice && <del>{formatBDT(product.compareAtPrice)}</del>}</div><p className={product.inStock ? 'stock' : 'out'}>{product.inStock ? 'In stock · ships in 1–2 days' : 'Out of stock'}</p><div className="card-actions"><button className="commerce-button" disabled={!product.inStock} onClick={() => addToCart(product)}><ShoppingBag />{product.inStock ? 'Add to cart' : 'Notify me'}</button></div></div>
  </article>
}
